...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut n = 0;
    let mut m = 0;
    let (mut start_poss, mut end_pos) = (Vec::new(), (0, 0));

    read(&mut n, &mut m, &mut map, &mut start_poss, &mut end_pos);
    ...
    for start_pos in start_poss {
        ...
        if vis[end_pos.0][end_pos.1] {
            min = min.min(val[end_pos.0][end_pos.1]);
        }
    }

    println!("The answer is: {}", min);
