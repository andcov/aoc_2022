...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let input = fs::read_to_string("input/day05_2.in")?;
    ...
    for instr in instructions {
        ...
        for _ in 0..(quant - 1) {
            new_last_ind = dll[new_last_ind].1;
        }

        let prev_el_ind = dll[new_last_ind].1;
        dll[prev_el_ind].2 = prev_el_ind;
        dll[last_ele[dest_col]].2 = new_last_ind;
        dll[new_last_ind].1 = last_ele[dest_col];
        last_ele[dest_col] = last_ele[sour_col];
        last_ele[sour_col] = prev_el_ind;
    }
